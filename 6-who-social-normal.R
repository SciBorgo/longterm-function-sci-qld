...
dsum = d %>% group_by(year) %>%
  summarise(mean = mean(who_social, na.rm = T))
d %>% ggplot() +
  geom_line(aes(x = year, y = who_social, group = participant_id), alpha = .2) +
...
tidy(fit, conf.int = T) %>%
  ...
  slice(-2) %>%
  write.csv("social-parameter-estimates.csv", row.names = F)
...
plot_data %>% ggplot() +
  ...
  labs(x = "Year", y = "WHO Social (0-100)") +
