...
dsum = d %>% group_by(year) %>%
  summarise(mean = mean(who_environment, na.rm = T))
d %>% ggplot() +
  geom_line(aes(x = year, y = who_environment, group = participant_id), alpha = .2) +
...
tidy(fit, conf.int = T) %>%
  ...
  slice(-2) %>%
  write.csv("enviro-parameter-estimates.csv", row.names = F)
...
plot_data %>% ggplot() +
  ...
  labs(x = "Year", y = "WHO Environment (0-100)") +
